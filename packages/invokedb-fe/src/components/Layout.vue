<template>
      ...
      <div class="header">
        <div class="left">
          ...
          <el-menu class="nav" ref="nav" mode="horizontal" @select="navSelected">
            ...
            <el-menu-item index="/guide" :class="{'is-active2': activeIndex === '/guide'}">
              <i class="menu-icon fa fa-file-alt"></i>
              <span class="menu-text">Guide</span>
            </el-menu-item>
            <el-menu-item index="/account" :class="{'is-active2': activeIndex === '/account'}">
  ...
  export default {
    ...
    setup() {
      ...
      const contact = () => {
        window.open(
          store.getters['env/HORIZON_AUTH_FE_PUB_HTTP_URL'] + '/contact',
          '_blank'
        );
      };
